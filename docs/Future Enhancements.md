...
- **Replace `WebSessionPayload` with `WebEventPayload`** — fields: `ip_hash`, `ga_hash`, `user_agent`, `referrer`, `language`, `page`, `status_code`, `event_type`, `event_name`, `event_data`, `target_url`, `timestamp`, `dnt`
- **Middleware**: post one event per request (event_type=page_view) instead of accumulating a session. Hash IP client-side with a daily salt before sending
